%% predictiveStanleyControlSFunction.m
%% stanley.slx模型的S-Function，实现预测stanley控制器功能
%% 作者：HPC2H2
%% 日期：20240507

function [sys, x0, str, ts] = predictiveStanleyControlSFunction(t, x, u,...
    flag, k_y, k_v, c, k0, Np)
switch flag
    case 0
    ...
    case 3
        sys = mdlOutputs(t, x, u, k_y, k_v, c, k0, Np);
    case {1, 4, 9}
        sys = [];
    otherwise
...
%%
    function sys = mdlOutputs(~, ~, u, k_y, k_v, c, k0, Np)
        %% 导入路径
        ...
        %% 预测模型
        % % 当前时刻Stanley模型输出转角
        [raw_delta, crossing_error, heading_error, pre_point, path_length] = ...
            computeDeltaErrorsAndPathInfo(pose, ref_poses, cur_v, k_y, k_v, c);

        % 预测矩阵初始化
        x_predict = [pose(1); zeros(Np-1, 1)];
        y_predict = [pose(2); zeros(Np-1, 1)];
        yaw_predict = [pose(3); zeros(Np-1, 1)];
        delta_predict = [raw_delta; zeros(Np-1, 1)];
        ...
        for i = 2:Np
            ...
            x_predict(i) = x_predict(i-1) + ...
                vx*cos(yaw_predict(i-1) + delta_predict(i-1))*Ts;
            y_predict(i) = y_predict(i-1) + ...
                vx*sin(yaw_predict(i-1) + delta_predict(i-1))*Ts;
            yaw_predict(i) = yaw_predict(i-1) +...
                vx*tan(delta_predict(i-1))/L*Ts;

            % 计算未来的delta
            cur_pose = [x_predict(i) y_predict(i) yaw_predict(i)];
            [delta_predict(i), ~, ~, predict_point, ~] = ...
                computeDeltaErrorsAndPathInfo(cur_pose, ref_poses, cur_v, k_y, k_v, c);

            % 预测步最近点为路径的终点
            if predict_point == path_length
                break
            end
        end

        % 第1~i（i最大为Np）个预测输出的权重
        k_predict = [k0; ones(i-1,1)*(1-k0)/(i-1); zeros(Np - i,1)]';

        % 计算预测Stanley转向角
        output_delta = k_predict * delta_predict;
